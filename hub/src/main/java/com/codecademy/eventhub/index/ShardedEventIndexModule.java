...
public class ShardedEventIndexModule extends AbstractModule {
  ...
  public EventIndex.Factory getEventIndexFactory(
      ...
      final @Named("eventhub.eventindex.initialNumEventIdsPerDay") int initialNumEventIdsPerDay,
       final EventIdList.Factory eventIdListFactory) {
     eventIdListFactory.setDefaultCapacity(initialNumEventIdsPerDay);
    return new EventIndex.Factory() {
      ...
      public EventIndex build(String eventType) {
        ...
        for (String date : dates) {
           eventIdListMap.put(date, eventIdListFactory.build(EventIndex.getEventIdListFilename(eventIndexDirectory, date)));
      }
    };
  }
  ...
  public ShardedEventIndex getShardedEventIndex(
    ...
    File file = new File(eventIndexFilename);
     try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
       ...
       SortedMap<String, Integer> eventTypeIdMap = (SortedMap<String, Integer>) ois.readObject();
       SortedMap<String, EventIndex> eventIndexMap = Maps.newTreeMap();
